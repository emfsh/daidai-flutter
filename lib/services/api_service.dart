...
class ApiService {
  ...
  String get baseUrl => _baseUrl ?? defaultPanelBaseUrl;

  Future<void> init() async {
    final prefs = await SharedPreferences.getInstance();
    _baseUrl = normalizeServerUrl(
      prefs.getString('server_url') ?? defaultPanelBaseUrl,
    );
  ...
  Future<http.Response> get(String path) async {
    final uri = Uri.parse('$baseUrl${apiPath(path)}');
  ...
  Future<bool> _tryRefreshToken() async {
    try {
      final uri = Uri.parse('$baseUrl${apiPath('/auth/refresh')}');
      final response = await http.post(
        uri,
  ...
  Future<Map<String, dynamic>> clearLogs() async {
    final response = await delete('/logs/clean');
    return jsonDecode(response.body);
  }
  ...
  Future<Map<String, dynamic>> clearLogsByDays(int days) async {
    final response = await post('/logs/clean', body: {'days': days});
    return jsonDecode(response.body);
  }
  ...
  Future<Map<String, dynamic>> getLoginLogs({int page = 1, int pageSize = 50}) async {
    final response = await get('/security/login-logs?page=$page&page_size=$pageSize');
    return jsonDecode(response.body);
  }
